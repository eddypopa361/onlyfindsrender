  ...
  return (
    <Card className="rounded-2xl overflow-hidden glow-card transition-all duration-300 card-zoom h-full border border-gray-900 shadow-[0_0_0_1px_#000]">
      ...
      <CardContent className="p-4 text-white">
        <Link href={`/product/${product.id}`}>
          <h3 className="font-heading font-semibold text-sm md:text-base mb-3 line-clamp-2 h-10 text-white cursor-pointer hover:text-primary transition-colors">
            {product.title}
          </h3>
        </Link>
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <a 
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-center px-3 py-2 border border-primary text-white font-heading font-medium rounded-md hover:bg-primary/10 transition-all text-xs"
            >
              View Details
            </a>
            <Link 
